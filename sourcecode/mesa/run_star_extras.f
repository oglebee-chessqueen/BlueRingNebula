...
! ***********************************************************************
      ...
      subroutine my_other_energy(id, ierr)
         use const_def, only: msol, standard_cgrav, pi, secyer, rsol
         integer, intent(in) :: id
         integer, intent(out) :: ierr
         type (star_info), pointer :: s
         real(dp) :: mabove, m_enclosed(1029), uheat_over_mp(1029), duration
         real(dp) :: m_planet
         integer :: i, k, nzones = 1029
         ierr = 0
         call star_ptr(id, s, ierr)
         if (ierr /= 0) return

         m_planet = 0.2*msol  ! for 0.2 Msol
!         m_planet = 0.0477763*msol  ! for 50 Mjup
         open (9, file = "profiles/mp_0.2msol/u_over_mp_radius_5.0_rsol.data")
         read (9, *)
         read (9, *)
         do i = 1, nzones
           read (9, *) m_enclosed(i), uheat_over_mp(i)
         end do
         close (9)
         s% extra_heat(:) = 0d0
         ...
         if (s%star_age*secyer < duration) then
           do k = 5, s% nz

             do i = 2, nzones-1
               if ( s%m(k)/msol > m_enclosed(i) ) then
                 s%extra_heat(k) = uheat_over_mp(i-1) &
                   + (s%m(k)/msol - m_enclosed(i-1) ) / (m_enclosed(i)-m_enclosed(i-1)) &
                     * (uheat_over_mp(i)-uheat_over_mp(i-1))
                 s%extra_heat(k) = s%extra_heat(k)*m_planet/ duration
                 write (*,*) i, k, s%m(k)/msol, m_enclosed(i-1), m_enclosed(i), s%extra_heat(k)
                 exit
               end if
             end do

             if (s%m(k)/msol .le. m_enclosed(nzones-1)) then
               s%extra_heat(k) = uheat_over_mp(nzones)*m_planet/duration
               write (*,*) nzones, k, s%m(k)/msol, m_enclosed(nzones), s%extra_heat(k)
             end if
